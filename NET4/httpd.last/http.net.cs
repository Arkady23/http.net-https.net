...
//!!   Головной блок              версия от 23.12.2025   !!
//!!                                                     !!
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
public class f : Form {
    ...
    private const string hn="http.net";
    ...
    public const string CL="Content-Length",CT="Content-Type",CD="Content-Disposition",
                 ...
                 Protocol="http", logX=hn+".x.log", logY=hn+".y.log",
                 CT_T=CT+": text/plain\r\n", stopIconText= hs+" is stopped",
                 initCGI= "initcgi.",
           //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                 ver="version 3.7.9", verD="December 2025";   //!!
           //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    private void RunServer(string[] args){
      ...
      if(getArgs(args)){
        ...
        // Неверные параметры запуска, показать параметры
        nIcon.Text = hs;
        this.Show();
        this.WindowState = FormWindowState.Normal;
        this.CenterToScreen();
