...
> [!NOTE]
> В версиях начиная с 3.4/1.4 будет поддерживаться только быстрый CGI. За счет этого интерпретатор скриптов не будет запускаться каждый раз перед выполнением каждого скрипта. Интерпретатор будет запущен только в начале работы сервера. Далее работу по обслуживанию запросов будет выполнять скрипт quickCGI. Пример такого скрипта на языке Python приведен в папке репозитория NET4/www/quickcgi.py. Это не относится к скриптам prg, у которых роль quickCGI выполняет COM-сервер VisualFoxPro.Application.
### General information
