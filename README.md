...
> [!NOTE]
> В версиях начиная с 3.4/1.4 будет поддерживаться только быстрый CGI. За счет этого интерпретатор скриптов не будет запускаться в момент поступления запроса клиента. Интерпретатор будет запускаться после окончания отправки ответа с скриптом initCGI. Пример такого скрипта на языке Python приведен в папке репозитория NET4/www/initcgi.py. Это не относится к скриптам prg, у которых роль initCGI выполняет COM-сервер VisualFoxPro.Application.
### General information
