...
Существуют весьма разнообразные обработчики. Среди них есть такие, которые не поддерживают многобыйтовые строки в UTF-8 кодировке, например, cscript.exe. Серверы http/https.net передают POST данные как есть, и если требуется перекодировка данных, то это должны делать сами скрипты.  

Если вы используете обработчик cscript с языком Jscript, то данные POST, если они содержат символы национальных алфавитов, вы можете отправлять на серверы http/https.net, используя преобразование: `STD_IO=encodeURIComponent(WINstring)`.
#### Обработчик VFP/VFPA
Для prg скриптов в качастве стандартного ввода-вывода для VFP серверы формируют объект _Screen.STD_IO на базе типа EditBox. Обычно входящий поток поступает в кодировке UTF-8. Поэтому prg скрипт должен конвертировать эти данные, находящиеся в _Screen.STD_IO, в свою внутреннюю системную кодировку. В VFP/VFPA для этого есть удобная функция Strconv(). Чтобы конвертировать весь входящий поток POST из UTF-8 используется обращение `STD_INPUT=Strconv(_Screen.STD_IO.value,11)`. Если у вас в данных POST не ожидаются буквы национального алфавита, а только цифры, буквы английского алфавита и стандартные знаки, то конвертировать вхдящий поток не обязательно.  
