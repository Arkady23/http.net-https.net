...
Если вы работаете с большими объемами памяти или предполагается большая нагрузка на VFP, то вместо очиски памяти, лучше использовать полное закрытие VFP командой quit. Команду quit вы можете поставить в конце prg-скрипта. В этом случае по окончанию сессии сервер перезапустит VFP.
